    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['search'])) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td>|</td>';
                echo "<th>" . htmlspecialchars($row['nim']!== null ? htmlspecialchars($row['nim']) : '') . "</th>";
                echo '<th>|</th>';
                echo "<th>" . htmlspecialchars($row['username']) . "</th>";
                echo '<th>|</th>';
                echo "<th>" . htmlspecialchars($row['majority']!== null ? htmlspecialchars($row['majority']) : '') . "</th>";
                echo '<th>|</th>';
                echo "<th>" . htmlspecialchars($row['yearofgraduation'] !== null ? htmlspecialchars($row['yearofgraduation']) : '') . "</th>";
                echo '<th>|</th>';
                echo "</tr>";
                echo "</table>";
                $counter++;
        ...
        <?php
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<td>|</td>';
                    echo "<td>" . htmlspecialchars($row['nim']!== null ? htmlspecialchars($row['nim']) : '') . "</td>";
                    echo '<td>|</td>';
                    echo "<td>" . htmlspecialchars($row['username']) . "</td>";
                    echo '<td>|</td>';
                    echo "<td>" . htmlspecialchars($row['majority']!== null ? htmlspecialchars($row['majority']) : '') . "</td>";
                    echo '<td>|</td>';
                    echo "<th>" . htmlspecialchars($row['yearofgraduation']!== null ? htmlspecialchars($row['yearofgraduation']) : '') . "</th>";
                    echo '<th>|</th>';
                    echo "</tr>";
